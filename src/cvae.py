...
class ConditionalVAE(nn.Module):
    ...
    def decode(
        self, latent_code: torch.Tensor, condition: torch.Tensor
    ) -> torch.Tensor:
...
class CNNDecoder(nn.Module):
    ...
    def __init__(self, input_shape: tuple, bottleneck_dim: int, num_classes: int):
        ...
        self.fc = nn.Sequential(nn.Linear(bottleneck_dim + 32, flat_dim), nn.ReLU(True))
...
class MLPEncoder(nn.Module):
    ...
    def __init__(self, input_shape: tuple, bottleneck_dim: int, num_classes: int):
        ...
        # Combine features and condition
        self.fc = nn.Sequential(nn.Linear(128 + 32, 2 * bottleneck_dim))
