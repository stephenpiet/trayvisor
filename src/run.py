...
def parse_args():
    ...
    parser.add_argument(
        '--conditional',
        action='store_true',
        help='Whether to use Conditional VAE'
    )
    parser.add_argument(
        '--num_classes',
        type=int,
...
def load_dataset(args):
    ...
    if args.dataset == 'mnist':
        ...
        if not args.image_dir or not args.annotations_file:
            raise ValueError("--image_dir and --annotations_file are required for plate dataset")
        ...
        for idx in train_dataset.indices:
            ...
            train_classes[class_label] = train_classes.get(class_label, 0) + 1
        for class_label, count in train_classes.items():
...
def main():
    ...
    try:
        ...
        logger.info(f"Best validation loss: {history['val_loss'][history['best_epoch'] - 1]:.4f}")
