...
class MyMNIST:
    def __init__(self):
        ...
        train_transforms = forms.Compose(
            [
                forms.Pad(2),
        ...
        test_transforms = forms.Compose(
            [forms.Pad(2), forms.ToTensor(), forms.Lambda(lambda x: 2 * x - 1)]
        )
        ...
        self.test_data = torchvision.datasets.MNIST(
            data_root, train=False, transform=test_transforms, download=True
        )
